...
func NewLDBDatabase(file string, cache int, handles int) (*LDBDatabase, error) {
	logger := logger.NewLogInstance("filedb", "debug", "", "")
	...
	logger.Info("Allocated cache and file handles", logger.Int("cache", cache), logger.Int("handles", handles))
...
func (db *LDBDatabase) Close() {
	...
	if db.quitChan != nil {
		...
		db.quitChan <- errc
		if err := <-errc; err != nil {
			db.log.Error("Metrics collection failed", db.log.String("err", err.Error()))
		}
	}
	err := db.db.Close()
	if err == nil {
		...
		db.log.Error("Failed to close database", db.log.String("err", err.Error()))
	}
}
